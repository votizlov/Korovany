...
namespace PathCreation
{
    ...
    public class BezierPath
    {
        ...
        {
            get { return (IsClosed) ? points.Count / 3 : (points.Count + 2) / 3; }
        }
        ...
        public void AddSegmentToEnd(Vector3 anchorPos)
        {
            ...
            if (controlMode != ControlMode.Mirrored && controlMode != ControlMode.Automatic)
            {
                ...
                secondControlForOldLastAnchorOffset =
                    (points[lastAnchorIndex] - points[lastAnchorIndex - 1]).normalized * dstPrevToNewAnchor * .5f;
            }
        ...
        public void SplitSegment(Vector3 anchorPos, int segmentIndex, float splitTime)
        {
            ...
            else
            {
                ...
                points.InsertRange(segmentIndex * 3 + 2,
                ...
                if (controlMode == ControlMode.Mirrored)
                {
                    ...
                    MovePoint(newAnchorIndex + 1, anchorPos + (splitSegment[1][1] - anchorPos).normalized * avgDst,
                        true);
                }
            }
                ...
                else
                {
                    ...
                    else if (controlMode != ControlMode.Free)
                    {
                        ...
                        if (attachedControlIndex >= 0 && attachedControlIndex < points.Count || isClosed)
                        {
                            ...
                            {
                                distanceFromAnchor =
                                    (points[LoopIndex(anchorIndex)] - points[LoopIndex(attachedControlIndex)])
                                    .magnitude;
                            }
        ...
        void UpdateClosedState()
        {
            if (isClosed)
            {
                ...
                if (controlMode != ControlMode.Mirrored && controlMode != ControlMode.Automatic)
                {
                    ...
                    lastAnchorSecondControl = points[points.Count - 1] +
                                              (points[points.Count - 1] - points[points.Count - 2]).normalized *
                                              dstBetweenStartAndEndAnchors * .5f;
                    firstAnchorSecondControl =
                        points[0] + (points[0] - points[1]).normalized * dstBetweenStartAndEndAnchors * .5f;
                }
